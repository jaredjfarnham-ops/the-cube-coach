...
(function () {
  ...
  /* ================================================================
     NON-WCA PUZZLES — tutorials + timer. No on-screen model yet (solve on your own puzzle).
     Methods are sourced from cubing references; depth/uncertainty is flagged per puzzle.
     ================================================================ */
  const nwTimer = (puz, title, scr) => ({ type:'trainer', solveOnly:true, puzzle:puz, title,
    intro:['A full-solve timer for the '+title.replace(/ Timer$/,'')+'. '+scr+' Apply the scramble to your own puzzle, hold <b>Space</b> and release to start, then press a key to stop. Times save with Ao5 / Ao12. <b>No on-screen model yet</b> — follow the scramble text.'] });
  const OFFICIAL = 'Scrambles are <b>official</b> (cubing.js random-state).';
  const APPDEF   = 'Scrambles are an <b>app-defined</b> random-move sequence — this is a non-WCA puzzle with no official scramble standard, so treat the notation as a starting point we can refine.';

  // ---- FTO ----
  LESSONS['fto/learn/about'] = { type:'text', title:'FTO — Face-Turning Octahedron', intro:[
    'The <b>Face-Turning Octahedron</b> (FTO) is an octahedron with 8 triangular faces; each <b>face</b> turns the ring of pieces around it. It has face centres, edges and corners.',
    'Notation uses the face letters with a <b>′</b> for a counter-clockwise turn. '+OFFICIAL ]};
  LESSONS['fto/learn/method'] = { type:'text', title:'FTO — Solving Method', intro:[
    'A common beginner approach works in layers, similar in spirit to a Pyraminx/megaminx hybrid:',
    '<ol><li>Solve one face’s <b>centre triangles</b> and the pieces around it.</li><li>Build the rest of that layer’s <b>edges</b>.</li><li>Solve the opposite layer’s centres/edges.</li><li>Orient then permute the <b>last-layer</b> edges and corners.</li></ol>',
    '<b>Note:</b> this is a high-level outline — pair it with a full video tutorial for the exact algorithms before relying on it.' ]};
  LESSONS['fto/timer/solve'] = nwTimer('fto','FTO Timer', OFFICIAL);

  // ---- CTO ----
  LESSONS['cto/learn/about'] = { type:'text', title:'CTO — Corner-Turning Octahedron', intro:[
    'The <b>Corner-Turning Octahedron</b> (CTO) is an octahedron that turns about its 6 <b>vertices</b> (corners) — the corner-turning dual of the FTO. It has 6 vertex pieces, 12 edges, and small trivial tips.',
    'Notation labels the vertices <b>L R F B U D</b> (like a cube), with <b>′</b> = counter-clockwise and <b>2</b> = half turn; tips turn independently. '+APPDEF ]};
  LESSONS['cto/learn/method'] = { type:'text', title:'CTO — Solving Method', intro:[
    'Jaap’s Puzzle Page gives a reliable phase-by-phase solution:',
    '<ol><li>Solve the trivial <b>tips</b>.</li><li>Solve the <b>vertex</b> (corner) pieces.</li><li>Place the <b>U-layer edges</b>.</li><li>Place the <b>middle-layer edges</b>.</li><li>Solve the <b>D vertex</b>.</li><li>Position the <b>D-layer edges</b>.</li><li>Orient (flip) the <b>D-layer edges</b>.</li></ol>',
    '<b>Shortcut:</b> twisting the vertices exactly as you’d solve a <b>Pyraminx</b> gets you roughly halfway, leaving a few edges to finish. Source: <i>Jaap’s Puzzle Page (jaapsch.net)</i>. Competitive CTO methods are informal.' ]};
  LESSONS['cto/timer/solve'] = nwTimer('cto','CTO Timer', APPDEF);

  // ---- Kilominx ----
  LESSONS['kilo/learn/about'] = { type:'text', title:'Kilominx — Overview', intro:[
    'The <b>Kilominx</b> is a <b>corners-only Megaminx</b> — the dodecahedral 2×2. 12 pentagonal faces, no edge pieces, so a face turn moves only corners.',
    'Notation is Megaminx-family face notation (named faces, <b>′</b> = counter-clockwise). '+OFFICIAL ]};
  LESSONS['kilo/learn/method'] = { type:'text', title:'Kilominx — Solving Method', intro:[
    'Solve it like a corners-only Megaminx, layer by layer:',
    '<ol><li>Build a <b>first face</b> of corners (one colour).</li><li>Complete the <b>first layer</b> (corners matched to their side colours).</li><li><b>Orient</b> the last-layer corners (get the top colour up).</li><li><b>Permute</b> the last-layer corners into place.</li></ol>',
    'The last-layer cases are a small subset of Megaminx corner algorithms.' ]};
  LESSONS['kilo/timer/solve'] = nwTimer('kilo','Kilominx Timer', OFFICIAL);

  // ---- Master Kilominx ----
  LESSONS['masterkilo/learn/about'] = { type:'text', title:'Master Kilominx — Overview', intro:[
    'The <b>Master Kilominx</b> is a 4-layer corners-only minx (the dodecahedral 4×4). Being even-layered it has <b>no fixed centres</b> and can show parity.',
    'Notation extends Megaminx <b>R++ / D-- / U</b> with inner-layer turns for the extra layers. '+APPDEF ]};
  LESSONS['masterkilo/learn/method'] = { type:'text', title:'Master Kilominx — Solving Method', intro:[
    'Solve by <b>reduction</b> to a Megaminx:',
    '<ol><li><b>Reduce centres</b> — pair the split centre facelets into solid centres (track colour order; no fixed centre to anchor to).</li><li><b>Pair edges</b> — join wing pieces into complete edges (4×4-style).</li><li><b>Solve as a Megaminx</b> with normal last-layer technique.</li><li><b>Parity</b> — there is no OLL parity, only a <b>PLL parity</b> (a double edge swap) to fix at the end.</li></ol>',
    'Source: Master Kilominx reduction tutorials (speedsolving / TheCubicle).' ]};
  LESSONS['masterkilo/timer/solve'] = nwTimer('masterkilo','Master Kilominx Timer', APPDEF);

  // ---- Gigaminx ----
  LESSONS['giga/learn/about'] = { type:'text', title:'Gigaminx — Overview', intro:[
    'The <b>Gigaminx</b> is a 5-layer Megaminx (the dodecahedral 5×5). It has fixed face centres surrounded by extra centre and edge pieces — 12 faces, many pieces.',
    'Notation is Megaminx face turns plus inner-layer turns for the deeper layers. '+APPDEF ]};
  LESSONS['giga/learn/method'] = { type:'text', title:'Gigaminx — Solving Method', intro:[
    'Solve by <b>reduction</b>, like a 5×5 into a 3×3:',
    '<ol><li><b>Solve the centres</b> — build the 12 solid face centres around the fixed centres.</li><li><b>Pair the edges</b> — combine each edge’s segments into a full edge.</li><li><b>Solve as a Megaminx</b> with standard last-layer algorithms.</li></ol>',
    'Source: Gigaminx reduction / “AI method” tutorials (allabouttwistypuzzles).' ]};
  LESSONS['giga/timer/solve'] = nwTimer('giga','Gigaminx Timer', APPDEF);

  // ---- Elite Kilominx ----
  LESSONS['elitekilo/learn/about'] = { type:'text', title:'Elite Kilominx — Overview', intro:[
    'The <b>Elite Kilominx</b> is a 6-layer corners-only minx (the dodecahedral 6×6) — edges are internal, so only centres and corners are visible. Even-layered, so no fixed centres and parity can occur.',
    'Notation extends Megaminx turns with multiple inner layers. '+APPDEF ]};
  LESSONS['elitekilo/learn/method'] = { type:'text', title:'Elite Kilominx — Solving Method', intro:[
    'Solve by <b>reduction</b> to a Kilominx:',
    '<ol><li><b>Reduce centres</b> on each face (track colour order — no fixed centres).</li><li><b>Group the layered corner blocks</b> so each corner behaves as one unit (no edges to pair).</li><li><b>Solve as a Kilominx/Megaminx</b> (corners-only last layer).</li><li><b>Resolve parity</b> from the even layer count.</li></ol>',
    '<b>Note:</b> structure is reliable, but a single authoritative step-by-step wasn’t confirmed — treat the exact steps as an outline.' ]};
  LESSONS['elitekilo/timer/solve'] = nwTimer('elitekilo','Elite Kilominx Timer', APPDEF);

  // ---- Teraminx ----
  LESSONS['tera/learn/about'] = { type:'text', title:'Teraminx — Overview', intro:[
    'The <b>Teraminx</b> is a 7-layer Megaminx (the dodecahedral 7×7) — 20 corners, 150 edges, 372 centres. Odd-layered, so it has fixed face centres.',
    'Notation is Megaminx face turns plus inner-layer turns up to layer 7. '+APPDEF ]};
  LESSONS['tera/learn/method'] = { type:'text', title:'Teraminx — Solving Method', intro:[
    'Solve by <b>reduction</b>, like a 7×7:',
    '<ol><li><b>Solve the centres</b> (innermost rings first, then outward) around the fixed centres.</li><li><b>Pair the edges</b> (three segments each) into full edges.</li><li><b>Solve as a Megaminx</b>. Being odd-layered, <b>no edge parity</b> arises.</li></ol>',
    'Source: Teraminx reduction references (grubiks / ruwix).' ]};
  LESSONS['tera/timer/solve'] = nwTimer('tera','Teraminx Timer', APPDEF);

  // ---- Master Pyraminx ----
  LESSONS['mpyra/learn/about'] = { type:'text', title:'Master Pyraminx — Overview', intro:[
    'The <b>Master Pyraminx</b> is a 4-layer Pyraminx. Beyond the tips and face turns it adds <b>inner-slice</b> turns, with extra edge and centre pieces.',
    'Notation: capital <b>U L R B</b> faces, lowercase <b>u l r b</b> tips, and wide/inner turns for the deeper layer. '+OFFICIAL ]};
  LESSONS['mpyra/learn/method'] = { type:'text', title:'Master Pyraminx — Solving Method', intro:[
    'Solve by <b>reduction</b> to a standard Pyraminx:',
    '<ol><li><b>Solve the centres</b> on each face.</li><li><b>Pair the edge (wing) pieces</b> into single edges.</li><li><b>Solve the reduced Pyraminx</b> with normal technique.</li><li><b>Parity</b> — fix the reduction parity if a lone edge looks flipped.</li><li><b>Twist the tips.</b></li></ol>' ]};
  LESSONS['mpyra/timer/solve'] = nwTimer('mpyra','Master Pyraminx Timer', OFFICIAL);

  // ---- Professor's Pyraminx ----
  LESSONS['profpyra/learn/about'] = { type:'text', title:"Professor's Pyraminx — Overview", intro:[
    'The <b>Professor’s Pyraminx</b> is a 5-layer Pyraminx (25 triangles per face) — the largest mass-produced pyraminx, designed by Timur Evbatyrov.',
    'Notation: <b>U L R B</b> faces, wide turns (<b>Uw</b>, <b>3Uw</b>…) for inner layers, lowercase tips. '+APPDEF ]};
  LESSONS['profpyra/learn/method'] = { type:'text', title:"Professor's Pyraminx — Solving Method", intro:[
    'Solve by <b>reduction</b> to a (Jing’s-style) 3-layer Pyraminx:',
    '<ol><li><b>Solve the centres</b> on all four faces.</li><li><b>Pair the edge segments</b> into complete edges.</li><li><b>Reduce</b> — the puzzle now behaves like a 3-layer Pyraminx with separate centres.</li><li><b>Solve the reduced Pyraminx.</b></li><li><b>Resolve parity</b> (an isolated flipped edge / centre swap).</li><li><b>Twist the tips.</b></li></ol>',
    'Source: Mefferts “A tutorial for the Professor Pyraminx” (reduction to Jing’s).' ]};
  LESSONS['profpyra/timer/solve'] = nwTimer('profpyra',"Professor's Pyraminx Timer", APPDEF);

  // ---- Royal Pyraminx ----
  LESSONS['royalpyra/learn/about'] = { type:'text', title:'Royal Pyraminx — Overview', intro:[
    'The <b>Royal Pyraminx</b> is a 6-layer Pyraminx (36 triangles per face), one step above the Professor — a collector-tier puzzle by Timur Evbatyrov.',
    'Notation follows the Pyraminx family with deeper wide/inner turns and tips. '+APPDEF ]};
  LESSONS['royalpyra/learn/method'] = { type:'text', title:'Royal Pyraminx — Solving Method', intro:[
    'Solve by <b>reduction</b>, same family approach as the Professor (more centre and edge orbits):',
    '<ol><li><b>Solve all centres</b> on each face.</li><li><b>Pair the edges</b> into single edges.</li><li><b>Reduce</b> to a 3-layer Pyraminx.</li><li><b>Solve the reduced Pyraminx.</b></li><li><b>Handle parity</b>, then <b>twist the tips.</b></li></ol>',
    '<b>Note:</b> the 6-layer reduction is demonstrated mainly on video — treat per-step algorithms as an outline.' ]};
  LESSONS['royalpyra/timer/solve'] = nwTimer('royalpyra','Royal Pyraminx Timer', APPDEF);

  // ---- Master Skewb ----
  LESSONS['masterskewb/learn/about'] = { type:'text', title:'Master Skewb — Overview', intro:[
    'The <b>Master Skewb</b> is a 2-cut (order-3) Skewb: each corner axis turns 120° but exposes an extra layer, adding triangular edges, diagonal centres and small “petal” pieces.',
    'Notation uses the Skewb corner twists <b>R L U B</b> (<b>′</b> = counter-clockwise), with wide turns for the inner cut. '+APPDEF ]};
  LESSONS['masterskewb/learn/method'] = { type:'text', title:'Master Skewb — Solving Method', intro:[
    'A beginner layer-by-layer method:',
    '<ol><li>Solve the <b>first-face edges</b>, then its <b>corners</b>.</li><li>Solve the <b>middle-layer edges</b>.</li><li>Solve the <b>last-layer edges</b>.</li><li>Solve the <b>centres</b>, then the <b>petals</b>.</li><li><b>Orient the last corners.</b></li></ol>',
    'Source: a single hobby-blog tutorial (afortmadeofbooks) — real but outline-level; pair with a video for exact algorithms.' ]};
  LESSONS['masterskewb/timer/solve'] = nwTimer('masterskewb','Master Skewb Timer', APPDEF);

  // ---- Elite Skewb ----
  LESSONS['eliteskewb/learn/about'] = { type:'text', title:'Elite Skewb — Overview', intro:[
    'The <b>Elite Skewb</b> is a 3-cut (order-4) Skewb — the deepest standard skewb (mf8). The cubic skewb series stops here: there is no standard 5/6/7-layer skewb.',
    'Notation uses Skewb corner twists <b>R L U B</b> with wide/inner-cut turns. '+APPDEF ]};
  LESSONS['eliteskewb/learn/method'] = { type:'text', title:'Elite Skewb — Solving Method', intro:[
    'The general approach is <b>reduction</b>: group the deeper-layer pieces (centres, petals, inner edges) to mimic a Master/standard Skewb, then finish with standard Skewb cycles.',
    '<b>Note:</b> no reliable written step-by-step was found for the Elite Skewb — this is an inferred outline. Follow a video tutorial for the exact algorithms before relying on it.' ]};
  LESSONS['eliteskewb/timer/solve'] = nwTimer('eliteskewb','Elite Skewb Timer', APPDEF);

  // ---- Redi Cube ----
  LESSONS['redi/learn/about'] = { type:'text', title:'Redi Cube — Overview', intro:[
    'The <b>Redi Cube</b> (Oskar van Deventer) is a cube whose 8 <b>corners</b> twist, cycling the 3 edges around each corner. Corners reorient in place but never move — the puzzle is really about the 12 edges.',
    'Notation (held like a Skewb): uppercase <b>R L F B</b> = the four top corners, lowercase <b>r l f b</b> = the four bottom corners; <b>′</b> = counter-clockwise. '+APPDEF ]};
  LESSONS['redi/learn/method'] = { type:'text', title:'Redi Cube — Solving Method', intro:[
  ...
  LESSONS['redi/timer/solve'] = { type:'trainer', solveOnly:true, puzzle:'redi', title:'Redi Cube Timer',
    intro:['A full-solve timer for the Redi Cube. '+APPDEF+' In <b>Virtual</b> mode you solve an interactive <b>3-D model</b> on screen — <b>drag</b> to turn it; the timer starts on your first move and stops the moment it’s solved. Or time a solve on your own puzzle with <b>Space</b>. Times save with Ao5 / Ao12.'] };
